...
func moveAction(validated move:ChessMove, game:inout ChessGameState) -> Effect<ChessAction>{
   print("We are appling \( game.board.whosTurnIsItAnyway)'s  move \(move)")
             ...
             //  game.board =
               ...
               let clearEffect = Effect<Output<ChessAction,ChessExoAction>>.sync(work: {
                   return .exo(.clear)
               })
               
               let nextTurnEffect = Effect<Output<ChessAction,ChessExoAction>>.sync(work: {
                   return .endo(.nextTurn)
               })
...
public func chessReducer(_ game:inout ChessGameState,_ action:ChessAction) -> Effect<ChessAction,ChessExoAction> {
